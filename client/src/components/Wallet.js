...
const Wallet = ({ user, addRemoveCash }) => {
  ...
  return (
    <>
      <div className="stat">
        <h2>Wallet</h2>
        <h3 data-testid="wallet-total" id="wallet-total">
          ${user.cash ? user.cash.toFixed(2).toString() : null}
        </h3>
        <h4>
          <button data-testid="add-money" id="add-money" onClick={handleClick}>
            Add Cash to Account
          </button>
        </h4>
        <form onSubmit={addRemoveCash} className="hidden" id="add">
          ...
          <input
            data-testid="wallet-form-submit"
            id="wallet-form-submit"
            type="submit"
          />
        </form>
      </div>
